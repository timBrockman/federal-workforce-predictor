...
(career trajectory, skill gaps, critical role readiness using assessments
+ synthetic career signals).
...
def get_recommendations(
    ...
    recs.append(
        {
            "category": "groceries",
            "suggested_monthly_budget": 280.0
            if questionnaire and "medium" in str(questionnaire.get("income_bracket", "")).lower()
            else 320.0,
            "confidence": 0.91,
        ...
        recs.append(
            {
                ...
                "confidence": 0.85,
                "rationale": (
                    f"Skills match: {', '.join(skills[:3])}. "
                    f"{profile.get('readiness_notes', '')}"
                ),
            }
        )
    if "cyber" in skills:
