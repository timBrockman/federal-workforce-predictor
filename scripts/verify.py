...
async def verify_db():
    ...
    async with factory() as session:
        ...
        q = await q_repo.create("verify-user", "medium", "save for travel", "medium", True)
...
async def verify_mcp():
    ...
    res_low = await mcp_call_tool(
        "get_budget_recommendations", {"user_id": "low-consent-user", "consent_level": 0}
    )
    ...
    res_submit = await mcp_call_tool(
        "submit_assessment",
        {
    ...
    async with stdio_client(server_params) as (read, write), ClientSession(read, write) as session:
        ...
        await session.call_tool("get_spend_summary", {"user_id": "verify-user-123"})
        print("  get_spend_summary via stdio (explicit user): OK")
        # Also exercise ask with context
        await session.call_tool(
            "ask_budget_agent", {"question": "coffee budget?", "consent_level": 1}
        )
        print("  ask_budget_agent via stdio (consent=1): OK")
        # Exercise new career tool via stdio
        await session.call_tool(
            "get_career_recommendations", {"user_id": "verify-user-123", "consent_level": 2}
        )
        print("  get_career_recommendations via stdio: OK")
